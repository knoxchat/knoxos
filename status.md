...
8. Async executor loop: keyboard, mouse, ~60 FPS redraw. Idle kernel thread `HLT`s when the desktop has no work.

### Architectural blockers (must fix first)

| Blocker | Evidence | Why it blocks a perfect OS |
|---------|----------|----------------------------|
| **No Ring 3 execution** | `jump_to_user_mode` has **zero callers**. `KERNEL_GS_BASE` is programmed. | Cannot run isolated programs. Desktop apps are kernel functions. |
...
| 3 | Process & Scheduling | Wired | 38% | **Critical** | Kernel-thread RIP switch + idle HLT; no Ring 3. |
...
**QEMU desktop demo readiness: ~70%** (boots, paints, clicks, types).
**Production OS readiness: ~36%** (kernel threads switch RIP; still cannot run isolated userspace).
...
**Grade: Wired (38%)** · `scheduler.rs`, `process.rs`, `context.rs`, `usermode.rs`, `signals.rs`

Kernel threads can switch RIP. Ring 3 is still not entered.
...
- [x] **`KERNEL_GS_BASE` / `GS_BASE`** programmed for `syscall` `swapgs` (not yet used from Ring 3)
- [x] **`deliver_signals`** called from `deferred_schedule` (no-op until a process has pending signals)

### Stub (control plane)
- [ ] **`iretq` to Ring 3** — `jump_to_user_mode` never called
...
3. First userspace: map a static `hello` ELF, `iretq`, `sys_write` to serial, `sys_exit`.
4. Then `fork` + `execve` + `waitpid` + SIGCHLD + Ctrl+C via PTY.

Until step 3 works, **do not add more scheduler policies**.
...
**Grade: Stub→Wired (18%)** · `elf.rs`, `dynlink.rs`, `syscall/mod.rs`, `vdso.rs`
...
### Stub
- [ ] **`execve` starts the binary** (`iretq` / `sysretq`)
...
### Perfect-OS next steps
Ship **static musl hello** first. Dynamic linking is Phase 2 of userspace, not Phase 1.
...
| ID | Task | Done when |
|----|------|-----------|
| B1 | TSS + `KERNEL_GS_BASE` + syscall entry that cannot fault on `swapgs` | **GS MSRs programmed**; still untested until Ring 3 `syscall` |
| B2 | Map static ELF, `iretq`, `sys_write` serial, `sys_exit` | QEMU serial prints `hello from userspace` |
...
**Production OS: ~36%** · **QEMU desktop demo: ~70%**

```
Kernel Core:        ███████████████░░░░░░░░░░  64%  Wired
Memory Mgmt:        ████████████░░░░░░░░░░░░░  48%  Wired
Process/Sched:      █████████░░░░░░░░░░░░░░░░  38%  Wired          ← critical path
Filesystem:         ██████████░░░░░░░░░░░░░░░  42%  Wired
...
AI/ML:              █████░░░░░░░░░░░░░░░░░░░░  22%  Wired
Binary Compat:      ████░░░░░░░░░░░░░░░░░░░░░  18%  Stub→Wired   ← critical path
i18n & Fonts:       █████████████████░░░░░░░░  68%  Live
...
| Process | 90% | 38% | Kernel-thread RIP switch is real; still no Ring 3 |
| Binary compat | 40% | 18% | 452 numbers ≠ 452 behaviors |
...
| **Overall production** | **~40%** | **~36%** | Recalibrated, then Gate A/G wired; still no userspace |
